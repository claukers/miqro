...
mainCMD({
  ...
  ["start"]: {
    ...
    cb: start,
    description: "\t\t\t\tstart a nodejs script in cluster mode and restart if crash."
  },

  ["watch"]: {
    //section: "watch",
    cb: watch,
    description: "\t\t\t\twatch a folder for changes and runs a command if a change occours."
  },
  ...
  ["doc"]: {
    ...
    cb: apiDocJSON,
    description: "\t\t\t\toutputs to stdout an api folder auto doc as a json"
  },
  ...
  ["db:console"]: {
    ...
    cb: consoleCMD,
    description: "\t\t\truns a readline interface that send the input as a query"
  },
