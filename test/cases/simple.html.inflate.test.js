...
describe("simple html inflate", () => {
  ...
  it("case 2 editor: false hotreload:true", async () => {
    ...
    strictEqual(response.ok, true);
    const data = (await response.text()).split(" ").map(h => h.trim().split("\n").map(h => h.trim()).join("")).join("");
    strictEqual(data, '<!DOCTYPEhtml><html><body><p>Hello&#32;World&#33;</p></body></html><script>//CreateWebSocketconnection.functiongetSocket(){returnnewWebSocket("/hot-reload");}constsocket=getSocket();lettimeout;functiontryConnection(){try{constnewSocket=getSocket();newSocket.addEventListener("open",(event)=>{console.log("reloading");setTimeout(()=>{window.location.reload();},500);});newSocket.addEventListener("error",(err)=>{console.error(err);timeout=setTimeout(tryConnection,500);});}catch(e){console.error(e);timeout=setTimeout(tryConnection,500);}}//Connectionclosedsocket.addEventListener("close",(event)=>{clearTimeout(timeout);timeout=setTimeout(tryConnection,500);});socket.addEventListener("error",(err)=>{console.error(err);});</script>');
    strictEqual(response.ok, true);
  });
